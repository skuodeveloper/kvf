...
public interface PesonStatisticsMapper extends BaseMapper<PesonStatistics> {
     ...
     * @param inviteCode
     */
    @Select ("call showMemberNodes(#{inviteCode})")
    void createMemberTempTable(@Param ("inviteCode") String inviteCode);
     ...
     * 获取部门统计的几代数据
     */
    @Select ("SELECT nLevel, COUNT(nLevel) count FROM tempmembernodes\n" +
            "GROUP BY nLevel\n" +
            "ORDER BY nLevel")
    List<LevelCount> getMemberLevelCount();
